...
```{r}
q1<-data.frame(brfss2013$X_state,brfss2013$dispcode) #makes df for Research question #1
q1<-q1[complete.cases(q1), ] #gets rid of NAs

head(q1,n = 5)
```
This is the data frame that will be used to answer the first question - Are people in some states more likely to complete the interview than others?

```{r}
tbq1<-q1%>% #use this to graph
group_by(brfss2013.X_state)%>%
summarise(dispct = sum(brfss2013.dispcode=='Completed interview')*100/n())
...
This shows that this data is slightly skewed to the left. This is verified by both the fact that the median is higher than the mean and that the graph is more bunched up towards the right hand side of the graph below.

```{r}
ggplot(data=tbq1,aes(x=brfss2013.X_state, y=dispct))+geom_bar(stat='identity', fill='#99badd', colour='black') + geom_hline(aes(yintercept=mean(dispct)))

ggplot(data=tbq1, aes(x=dispct)) + geom_histogram(bins=50)

```

It should be noted that California is an outlier with 75.1% of its surveys completed in full.
...
ggplot(data=q2tib,aes(x=brfss2013.X_state, y=fruit_consumpt))+geom_bar(stat='identity', width = 0.5,fill = "#99badd",colour='black' ) + geom_hline(aes(yintercept=mean(fruit_consumpt)))
...
```{r}
ggplot(data=tibDFtest,aes(x=brfss2013.veteran3, y=alcperct))+geom_bar(stat='identity', width = 0.5,fill = "#99badd",colour='black' )
